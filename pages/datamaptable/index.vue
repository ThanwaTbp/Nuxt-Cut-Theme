<template>
	<div class="container-fluid">
		<div class="content d-flex flex-row-fluid row" id="kt_content">
			<div id="kt_content_container" class="container-fluid col-lg-7">
				<div class="card card-flush">
					<div class="card-header align-items-center py-5 gap-2 gap-md-8">
						<!--begin::Card title-->
						<h1>Road Surface Data</h1>
			...
			<div class="d-flex justify-content-center col-lg-5">
				<MapLongDoMap @loaded="loadMapAlignment" />
			</div>
		</div>
	</div>
</template>
...
const headers: Header[] = [
	...
	{ text: "ID", value: "id" },
	{ text: "KmStart", value: "kmStart" },
	{ text: "KmEnd", value: "kmEnd" },
	{ text: "Latitude", value: "lat", sortable: true },
	{ text: "Lontitude", value: "lon", sortable: true },
	{ text: "Operation", value: "operation" },
];

// const items = ref<Item[]>([]);
const items = positionStore.positionData.map((item) => item);
...
const showRow = (item: ClickRowArgument) => {
	positionStore.changePositions(item);
};

const loadMapAlignment = (longdo, map) => {
	longdo.value = longdo;
	map.value = map;

	const line = positionStore.positionData.map((data) => {
...
const loadFromApi = async () => {
	...
	// let url = `https://api.coincap.io/v2/assets?limit=${rowsPerPage}&offset=${
	// 	(page - 1) * rowsPerPage
	// }`;
	// if (sortBy == "undefined") {
	// 	url = url;
	// } else {
	// 	url = `https://api.coincap.io/v2/assets?limit=${rowsPerPage}&offset=${
	// 		(page - 1) * rowsPerPage
	// 	}&search=${sortBy}`;
	// }
